...
export type PostVariant = {
    ...
    object: 'post_variant';
    /**
     * The post shape, DERIVED from the media (the customer never sends it). null only while the shape is undetermined — referenced media is still being probed (kind not yet known), or the current media/settings combination has no valid shape for this platform. (A text-capable platform with zero media derives `text`, not null.)
     */
...
export type MediaCompleteErrors = {
    ...
    422: {
         ...
         * The machine-readable error code — branch on this. Narrowed to the codes this endpoint can return at this status.
         */
        code: 'idempotency_key_reused' | 'media_upload_invalid_parts';
        display_error: string;
        detail?: string;
        request_id?: string;
        source?: ErrorSource;
