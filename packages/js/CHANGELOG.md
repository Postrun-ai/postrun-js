...
- d1e68e9: Automatic token management. The SDK now caches the scoped token and refreshes it reactively, so `getToken` can be a trivial "fetch a token from my backend" — it is no longer called per request, and customers no longer cache or track token expiry themselves.
